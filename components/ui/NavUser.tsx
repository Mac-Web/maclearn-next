...
const menuIconStyles = "group-hover:scale-120 group-hover:-translate-y-1 transition-all! duration-300";
    ...
    <div className="relative">
      ...
      {menuOpen && (
        <div className="absolute right-0 top-[120%] flex flex-col rounded-lg p-2 bg-gray-100 dark:bg-gray-900 w-35" ref={menuRef}>
          ...
          >
            <FaUser size={20} className={menuIconStyles} />
            Profile
          </a>
          <div className={menuOptionStyles} onClick={() => setTheme(theme === "light" ? "dark" : "light")}>
            {theme === "light" ? (
              <MdLightMode size={20} className={menuIconStyles} />
            ) : (
              <MdDarkMode size={20} className={menuIconStyles} />
            )}
            Theme
          </div>
          <div className={menuOptionStyles} onClick={() => signOut()}>
            <MdLogout className={menuIconStyles} size={20} />
            Log out
          </div>
        </div>
      )}
